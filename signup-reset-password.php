		...
		if (isset($resetUser) && !empty($resetUser['user_id'])) { // user id should be retrieved after token is validated successfully
			/*RAW QUERY
				...
				WHERE
					user_id = 1;
			*/
			$reset_password_query = "UPDATE user_details SET `password` = '$hashedPassword', `status` = 'Approved', reset_token_hash = NULL, reset_token_expiry = NULL WHERE user_id = " . (int)$resetUser['user_id'];
